...
| `chat.py` | OpenAI-совместимый HTTP, model service/request mode, SSE/tokenizer; controlled completion headers и polling socket reader | один первый system; timeout; локальный ключ; checkpoint headers/body в controlled completion; legacy openers требуют отдельной проверки |
