...
- [ ] AEC/full-duplex — **PARTIAL**: единый near/far 10-мс worker, opt-in PCM output, закреплённый WebRTC AEC3/NS и строгий запрет SAPI fallback готовы. Tour One M3 живьём автовыбран; Xenia PCM завершилась успешно, cancellation подтверждён за 7,2 мс, три AEC/far handshakes стабильны после исправления Windows pipe. Далее: synthetic echo A/B → реальный JBL self-echo AEC off/on → произвольный speech-start barge-in → продолжение следующей реплики без wake-фразы → 20–30 turns. `live.enabled` не включать до измеримого выигрыша.
