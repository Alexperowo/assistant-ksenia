...
- [x] 428 автоматических тестов, контроль количества и предупреждений, три случайных порядка, полный контракт ярлыков, performance tracing, tool lifecycle/cancellation, fast resident tier, автовыбор реально открывающегося речевого микрофона, near/far PCM, fail-closed AEC-конфигурация, runtime cache/profiles и регрессии запрета модельного/машинного хардкода.
...
- [ ] AEC/full-duplex — **PARTIAL**: единый near/far 10-мс worker, opt-in PCM output, закреплённый WebRTC AEC3/NS и строгий запрет SAPI fallback готовы. Tour One M3 живьём автовыбран; Xenia PCM завершилась успешно, cancellation подтверждён за 7,2 мс, а физический A/B не обнаружил self-echo даже без DSP. Input block измеренно снижен с 250 до 40 мс. Далее: человеческая речь во время TTS → произвольный speech-start barge-in → продолжение следующей реплики без wake-фразы → 20–30 turns; synthetic echo/cалибровка нужны только при подтверждённом эхе. `live.enabled` не включать по умолчанию до этой приёмки.
