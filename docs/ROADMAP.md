...
- [ ] Реализовать непрерывное слушание после обычного ответа/ошибки (C3), затем
  независимый фоновый поиск (C4). Сейчас повторная wake-активация всё ещё нужна;
  `live.enabled` остаётся выключенным. Не включать boolean вместо исправления цикла.
