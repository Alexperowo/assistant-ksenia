...
Изменение принималось только после живых измерений. Основные функциональные роли пока не переключены на новые профили: сначала внедрены управляемый runtime, request modes, строгий протокол предложения UI-действия и независимая проверка.
...
Это подтверждает совместное проживание именно малой пары. Инвариант «одна крупная модель» сохраняется: перед будущим запуском Laguna/Qwen пара должна безопасно освобождать память и восстанавливаться после завершения тяжёлой задачи.
...
- Две малые модели не запускаются рядом с большой без residency coordinator и измерения освобождения/восстановления VRAM.

Следующая приёмка: реальный Windows screenshot capture, отрицательные multi-monitor/DPI тесты, 20–30 read-only UI proposals, затем один подтверждённый безопасный action через существующий Permission Broker. До этого новые профили остаются экспериментальными и не заменяют рабочие роли.
