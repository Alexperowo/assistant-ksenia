...
- 356 unit/integration-тестов и три дополнительных прогона в случайном порядке;
...
Новый Ornith APEX MTP Fixed закреплён отдельным выключенным кандидатом; результаты старого A/B другого Ornith-артефакта к нему не переносятся. После завершения загрузки ему нужны собственные MTP on/off и функциональные замеры при 16K. Штатный `ModelManager` уже подтвердил оба рабочих контура при 96K: Laguna+DFlash на PoolSide и Qwen+MTP+mmproj на официальном b10621. Общий `AUDIT.cmd` прошёл с активной Laguna, LAN и Silero→Whisper; RAG live-test честно пропущен, потому что RAG выключен текущей конфигурацией. Для полного модельного допуска ещё нужны tool/vision-сценарии. Также остаются ручные проверки физического микрофона и динамиков, AEC, Bluetooth-кнопок, телефона в домашней сети и действий на настоящих сайтах. Точный статус находится в [ROADMAP](docs/ROADMAP.md) и [аудите](docs/AUDIT.md).
...
Текущий каталог содержит три профиля: `generalist` — Laguna XS 2.1 APEX I-Quality с отдельной DFlash-моделью на PoolSide backend; `reasoning` — Qwen 3.8 27B Opus Distill v2 с MTP и multimodal projector на официальном backend; `candidate` — выключенный Ornith 1.5 35B-A3B на официальном backend. Имена, backend, источники, commit, размеры, SHA-256 и параметры запуска находятся только в декларативной конфигурации. Исполняемый код не содержит ветвлений по названию модели и абсолютных путей конкретного компьютера.
