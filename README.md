...
- 326 unit/integration-тестов и три дополнительных прогона в случайном порядке;
- официальный `llama.cpp b10241`, CUDA 12.4, с закреплёнными SHA-256;
...
Новый Ornith APEX MTP Fixed закреплён отдельным выключенным кандидатом; результаты старого A/B другого Ornith-артефакта к нему не переносятся. После завершения загрузки ему нужны собственные MTP on/off и функциональные замеры при 16K. Laguna+DFlash уже подтверждена на 96K в отдельном локальном runtime; штатному профилю Ксении на официальном движке и Qwen+MTP ещё нужна живая приёмка в свободное GPU-окно. Также остаются ручные проверки физического микрофона и динамиков, AEC, Bluetooth-кнопок, телефона в домашней сети и действий на настоящих сайтах. Точный статус находится в [ROADMAP](docs/ROADMAP.md) и [аудите](docs/AUDIT.md).
...
Текущий каталог содержит три профиля: `generalist` — Laguna XS 2.1 APEX I-Quality с отдельной DFlash-моделью; `reasoning` — Qwen 3.8 27B Opus Distill v2 с MTP и multimodal projector; `candidate` — выключенный Ornith 1.5 35B-A3B. Имена, источники, commit, размеры, SHA-256 и параметры запуска находятся только в декларативной конфигурации. Исполняемый код не содержит ветвлений по названию модели и абсолютных путей конкретного компьютера.
