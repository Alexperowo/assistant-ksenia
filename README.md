...
| Локальные модели | Тёплая пара малых GGUF либо одна крупная GGUF через `llama.cpp`, переключение по ролям |
| Роли | Быстрый дворецкий, быстрый исследователь, исполнитель, планировщик и тяжёлый мозг |
...
- **Предсказуемая резидентность:** UI-Mate и Agents-A1 могут жить одной измеренной парой; перед любой крупной моделью пара освобождает VRAM и затем восстанавливается целиком.
...
- 447 unit/integration-тестов и три дополнительных прогона в случайном порядке;
...
Ornith conFIGur8tor Compact и Ornith Quality Q6_K прошли одинаковый семисценарный A/B: оба дали 7/7, Compact использовал заметно меньше VRAM, а MTP conFIGur8tor на n=1/2/4 не ускорил ответы. Дополнительный полный agent/tool gate Compact завершил правильным чтением активного окна за 17,0 секунды cold и 1,6 секунды при повторном prefix; поэтому Compact принят исполнителем среднего уровня без MTP. Laguna S остаётся выключенным кандидатом: DFlash n=5 на текущем UD-IQ3_S замедлил корпус. Qwen MTP n=2 сохранил измеримый выигрыш. Полные данные и карантин локального SC117-подобного файла: [MODEL-RUNTIME-AB-2026-08-30](docs/MODEL-RUNTIME-AB-2026-08-30.md). Также остаются ручные проверки физического Live, телефона и действий на настоящих сайтах.
...
Рабочая лестница задаётся функциональными ролями, а не именами в коде. `ui_butler` (UI-Mate) отвечает за быстрый разговор, `research_fast` (Agents-A1) — за веб-исследование, `candidate` (Ornith Compact) — за обычное инструментальное исполнение, `reasoning` (Qwen 3.8) — за усиленное планирование, `generalist` (Laguna XS) — за редкий тяжёлый long-context режим. `heavy_candidate` Laguna S и `quality_candidate` Ornith Quality остаются выключенными кандидатами. Имена, backend, источники, commit, размеры, SHA-256 и параметры запуска находятся только в декларативной конфигурации. Исполняемый код не содержит ветвлений по названию модели и абсолютных путей конкретного компьютера.
